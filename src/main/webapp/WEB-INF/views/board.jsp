...
<div class="container">
    ...
    <form action="" method="post" id="form" class="frm">
        ...
        <input name="title" type="text" value="<c:out value='${boardDto.title}'/>" placeholder="제목을 입력해주세요." ${mode=="new"? "": "readonly='readonly'"}><br>
        <textarea name="content" rows="20" placeholder="내용을 입력해주세요." ${mode=='new'? "":"readonly='readonly'"}><c:out value="${boardDto.content}"/></textarea><br>
...
<script>
    ...
    return true; //제목과 내용 비어있지 않도록 검증
}
    ...
    //1. 현재 읽기 상태이면, 수정 form으로 변경
    if(isReadonly=='readonly'){
    $(".writing-header").html("게시물 수정");
    $("input[name=title]").attr('readonly', false);
    $("textarea").attr('readonly',false);
    $("#modifyBtn").html("<i class='fa fa-pencil'></i> 등록");
    return; //form submit하지 않고 여기서 return.
}

    //2.수정 form이면, 수정한 내용을 서버로 전송
